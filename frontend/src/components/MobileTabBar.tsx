...
export function MobileTabBar() {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 md:hidden px-4 pb-4 pt-2 bg-gradient-to-t from-background via-background to-transparent pointer-events-none [transform:translateZ(0)]">
      <div className="flex items-center justify-around bg-white/80 backdrop-blur-xl border border-gray-200/50 rounded-3xl p-2 shadow-[0_8px_30px_rgb(0,0,0,0.12)] pointer-events-auto">
        {tabs.map((tab) => {
          ...
          return (
            <Link key={tab.name} href={tab.href} className="relative flex-1 flex flex-col items-center justify-center p-2 rounded-2xl transition-colors">
              <motion.div
                whileTap={{ scale: 0.9 }}
                className={`relative z-10 flex flex-col items-center justify-center gap-1 ${isActive ? "text-emerald-600" : "text-gray-400 hover:text-gray-600"}`}
              >
                <div className="relative">
                  <Icon className="w-6 h-6" strokeWidth={isActive ? 2.5 : 2} />
                  {tab.badge !== undefined && tab.badge > 0 && (
                    <span className="absolute -top-1.5 -right-1.5 bg-accent text-white text-[10px] font-bold w-4 h-4 flex items-center justify-center rounded-full ring-2 ring-white">
                      {tab.badge}
                    </span>
                  )}
                </div>
              ...
              {isActive && (
                <motion.div
                  ...
                  className="absolute inset-0 bg-emerald-50 rounded-2xl z-0"
