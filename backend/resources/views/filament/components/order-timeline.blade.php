@php
    $status = $getRecord()?->status ?? 'pending';
    if ($status === 'pending_payment') {
        $status = 'pending';
    }
    
    $steps = [
        'pending' => ['label' => 'Pendiente', 'icon' => 'heroicon-m-clock'],
        'processing' => ['label' => 'Procesando', 'icon' => 'heroicon-m-cog'],
        'shipped' => ['label' => 'Enviado', 'icon' => 'heroicon-m-truck'],
        'delivered' => ['label' => 'Entregado', 'icon' => 'heroicon-m-check-badge'],
    ];
...
<div style="width: 100%; padding-top: 1.5rem; padding-bottom: 1.5rem;">
    @if($status === 'cancelled')
        <div style="display: flex; align-items: center; justify-content: center; gap: 0.5rem; color: #dc2626; background-color: #fef2f2; padding: 1rem; border-radius: 0.75rem;">
            ...
            <span style="font-size: 1.125rem; font-weight: bold;">Este pedido fue CANCELADO</span>
        </div>
    @else
